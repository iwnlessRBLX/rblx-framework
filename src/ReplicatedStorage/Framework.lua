...
function ProcessCall(sName, index, indexType, args)
	...
	if RunService:IsServer() then
		...
		elseif indexType == "FireUnreliable" then
			if typeof(player)=="table" then
				for i,v in pairs (player) do
					if not v:IsA("Player") then
						continue
					...
					script.UnreliableRemoteEvent:FireClient(v, sName, index, args)
				end
			elseif player:IsA("Player") then
				script.UnreliableRemoteEvent:FireClient(player, sName, index, args)
			end
		elseif indexType == "FireAllUnreliable" then
			script.UnreliableRemoteEvent:FireAllClients(sName, index, args)
		end
		...
		elseif indexType == "FireUnreliable" then
			script.UnreliableRemoteEvent:FireServer(sName, index, args)
		end
	end
...
function module.Start(folder: Folder|{any})
	...
	if RunService:IsServer() then
		...
		script.UnreliableRemoteEvent.OnServerEvent:Connect(callback)
	else
		...
		script.UnreliableRemoteEvent.OnClientEvent:Connect(callback)
	end
