...
local function getNonHumanoidWaypoint(self)
	...
	for i = 2, #self._waypoints do
		if (self._waypoints[i].Position - self._waypoints[i - 1].Position).Magnitude > 0.25 then
			return i
		end
	end
	return 2
...
local function comparePosition(self)
	...
	if self._position._count >= self._settings.COMPARISON_CHECKS and (self._humanoid.Parent.PrimaryPart.Anchored == false) then
		if self._settings.JUMP_WHEN_STUCK then
			if (Cooldown[self._agent.Name] == nil) then
				warn("stuck",self._agent.Name)
				Cooldown[self._agent.Name] = true
				setJumpState(self)
				self._humanoid:MoveTo(self._agent.PrimaryPart.CFrame.LookVector * -3)
				task.wait(0.5)
				Cooldown[self._agent.Name] = nil
			end
		end
...
function Path:Stop()
	...
	self._humanoid.WalkToPoint = Vector3.new()
	disconnectMoveConnection(self)
	self._status = Path.StatusType.Idle
	self._visualWaypoints = destroyVisualWaypoints(self._visualWaypoints)
...
local function SetLastWaypointNumber(self, val)
	self._WaypointNumber = val
end

local function SetLastTick(self, val)
	self._Tick = val
end

function Path:Run(target, Tick)
	...
	if (self._path.Status == Enum.PathStatus.NoPath) then
		local Att = 0
		repeat
			pathComputed, _ = pcall(function()
				self._path:ComputeAsync(self._agent.PrimaryPart.Position, (typeof(target) == "Vector3" and target) or target.Position)
			end)
			Att += 1
			task.wait()
		until (self._path.Status ~= Enum.PathStatus.NoPath) or (Att >= 5)
	...
	--print(self._path.Status == Enum.PathStatus.NoPath,
	--	self._humanoid and self._humanoid:GetState() == Enum.HumanoidStateType.Freefall,
	--	not pathComputed
	--)
	...
	--Begin pathfinding
	if (self._humanoid.Parent.PrimaryPart.Anchored == true) then
		repeat task.wait() until (self._humanoid.Parent.PrimaryPart.Anchored == false)
	end

	local highest_index = -1

	for i, v in pairs(self._waypoints) do
		if (i > 5) then
			break
		end
		local nextWaypoint = self._waypoints[i+1]
		if (nextWaypoint) then
			if (v.Position - nextWaypoint.Position).Magnitude > (self._humanoid.Parent.PrimaryPart.Position - nextWaypoint.Position).Magnitude then
				--print("is behind",i)
				self._waypoints[i] = nextWaypoint
				highest_index = i+1
			end
		end
	end
	...
	if (highest_index ~= -1) then
		for i, v in pairs(self._waypoints) do
			if (i < highest_index) then
				self._waypoints[i] = self._waypoints[highest_index]
			else
				--warn("got to",i)
				break
			end
	...
	--if (#self._waypoints >= 4) then
	--	self._waypoints[3] = self._waypoints[4]
	--elseif (#self._waypoints >= 3) then
	--	self._waypoints[2] = self._waypoints[3]
	--end
	--self._waypoints[1] = self._waypoints[2]
	--self._currentWaypoint = 1
...
function Path:GetPath(target, Tick)
